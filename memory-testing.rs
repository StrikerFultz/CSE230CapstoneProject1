...
impl Memory {
    pub fn set_word(&mut self, address: u32, value: u32) {
        ...
        // Bounds-check: ensure writing a word doesn't overflow the page
        if offset + WORD_SIZE > PAGE_SIZE {
            // Out-of-bounds write; choose to ignore or panic. Here we panic with a helpful message.
            panic!("attempt to write word across page boundary: page={} offset={}", page, offset);
        }

...
impl Parser {
    ...
    pub fn parse_instruction(&mut self) -> Result<Instruction, EmuError> {
        ...
        if let Some(token_ref) = x {
            ...
            match lexeme.as_str() {
                "add" | "sub" | "or" | "addu" | "subu" | "and" | "xor" | "slt" | "sltu" | "mult" | "mflo" | "mfhi" | "div" | "nor" | "sll" | "srl" => self.parse_r_type(&lexeme),
    ...
    pub fn parse_r_type(&mut self, mnemonic: &str) -> Result<Instruction, EmuError> {
        ...
        match mnemonic {
            "add" | "sub" | "or" | "addu" | "subu" | "and" | "xor" | "slt" | "sltu" | "nor" => {
                ...
                match mnemonic {
                    ...
                    "nor" => Ok(Instruction::Nor {rd, rs, rt}),
                    _ => unreachable!()
                }
            },
            ...
            "srl" | "sll" => {
                ...
                let sa: u32 = sa_token.lexeme.parse::<u32>()
                    .map_err(|_| self.error(format!("Line {}: invalid shift amoint '{}'", sa_token.line_number, sa_token.lexeme)))?;

                match mnemonic {
                    "sll" => Ok(Instruction::Sll {rd, rt, sa}),
                    "srl" => Ok(Instruction::Srl {rd, rt, sa}),
                    _ => unreachable!()
                }
            }
